...
- **Built by talking.** The AI assistant that writes weft with you is part of the language, not a plugin you wire up: `weft new hello --assistant claude-code` (shorthand `cc`, one flag value per assistant as more arrive, the choice remembered for future projects) installs Tangle, and opening the project in Claude Code loads it automatically. Tangle knows the whole catalog, builds one stage at a time, runs it, reads the journal, and writes the nodes that are missing.
...
weft new hello --assistant claude-code && cd hello && weft run  # scaffold (with the tangle AI persona), compile, fire an execution
```
...
The `--assistant claude-code` in the quickstart did the other half: open the
same `hello` folder in Claude Code and Tangle is already there, the AI
builder that knows the language and the whole node catalog. You describe
what you want, it builds, runs, and shows you what happened. From now on
`weft new` remembers your assistant and installs it without the flag;
`--assistant none` opts back out.
