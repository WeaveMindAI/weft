...
| An incoming HTTP request | `Route` |
| An incoming WebSocket connection | `Socket` |
| Events from a connected provider | `ProviderEvents` |

`SocketListen` and `Socket` sound alike and are easy to swap by mistake.
`SocketListen` dials out to a service; `Socket` is what an outside caller
...
`Route` and `Socket` share `LiveConnectionConfig`: the route pattern, the
method, the body shape, the auth connection and the suspension defaults.
