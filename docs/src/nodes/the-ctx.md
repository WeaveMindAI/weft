...
If the node supports either protocol, `ctx.live_caller().await?` gives you
whichever one is connected, and `ctx.caller()` returns an optional
`CallerHandle`. `ctx.caller_request()?` is what the caller sent to open the
exchange. You can also ask `ctx.is_api_call()`, `ctx.is_websocket()` and
`ctx.caller_data_type()`.
