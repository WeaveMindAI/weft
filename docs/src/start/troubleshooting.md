...
## The CLI cannot reach the runtime, or port 9999 is taken
...
maps to your machine, the second is where the CLI looks, and a later `weft
daemon stop` in a shell missing the first goes hunting for a daemon on 9999.
...
cluster, and it says so before it does. Your system database survives that
rebuild; every project's own database lives inside the cluster and does not.
So pick the port before you have projects you care about. `WEFT_INGRESS_PORT`
(9998, storage downloads) and `WEFT_GATEWAY_PORT` (9097, live callers) work
the same way.
