...
SQL, its Python and its prompts from the files sitting beside `src/main.weft`.
...
`assets/sql/spend_credit.sql` takes one credit off, but only if the balance is above
zero, and reports why it did not when it did not. Then `assets/scripts/outcome.py`
