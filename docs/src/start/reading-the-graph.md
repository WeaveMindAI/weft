...
Open `main.weft` in VS Code and you get the code on the left and the graph on
the right.
...
is included, but the walk does not carry on past it. If you target a box inside
a group or a loop, the whole of that group or loop comes with it. For the exact
rules, read
