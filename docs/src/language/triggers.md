...
A `_should_flow` wire counts as downstream like any other, and on a group or
an included file it takes the whole group along, so a route can hand its work
to a group without wiring any data into it:

```weft
work._should_flow = live.method
```
