...
closure; read [running on the thing that did not
happen](syntax.md#running-on-the-thing-that-did-not-happen) for when to
reach for it.
...
when it reaches a trigger. A step that several branches share does not drag
those other branches in. A target inside a group brings only the work needed
through it, with the group's own `_should_flow` still applied. A loop is the
exception: it comes along whole, and cutting inside one is refused.
...
A value that a real execution produced beats one you handed in, and a
producer that is still running is waited for. Anything you supplied shows in
the editor as `provided by hand`. For which flag picks which part of the
graph, and how to fire a trigger by hand, read [Versions, seeded runs and
frozen examples](../running/versions.md#running-one-group-or-one-node-onward).
