...
For the actual work, weft is a Rust framework. You write a step's logic in Rust with the APIs weft gives you, and I'm not going to invent a new syntax for parsing a response or adding up a number. The language has one job: saying how the pieces fit together. For that job the syntax is the whole point.
...
Where I want to take it is proving things about how the program behaves, at compilation time. The idea is to let a node carry labels about what it is and what it does: a node that takes an action in the world says so, and how big that action is; a node that's an autonomous agent says so; a node that verifies something says what strength of verification it gives you. Then the compiler checks claims across the whole graph. Every path from an autonomous step to a high-stakes action goes through a verification of at least this strength, or a critical action has three separate human approvals in front of it. If the program doesn't hold up, it doesn't build.

You could even record how often a model hallucinates, and get the chance that a step's output contains one after chaining several AI steps. Add a recovery step, like a second verification with a web search, and that chance drops.
...
If something here looks wrong, or if you have other arguments, come tell me on [Discord](https://discord.com/invite/FGwNu6mDkU). Your argument could help shape weft into something more useful.
