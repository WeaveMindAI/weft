//! WhatsAppBridge self-tests: the declared infrastructure shape.

use serde_json::json;

use weft::{FakeRig, NodeTest, WeftResult};

use super::WhatsAppBridgeNode;
...
async fn provisions(rig: FakeRig) -> WeftResult<()> {
    let outcome = rig.run_provision_infra(&WhatsAppBridgeNode, json!({})).await.ok()?;
