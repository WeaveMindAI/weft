  ...
  async function connect() {
    ...
    sock.ev.on('messages.upsert', async ({ type, messages }) => {
      ...
      for (const msg of messages) {
        ...
        // TEMP DEBUG: dump the raw message shape so we can see why
        // real messages from other phones are hitting the "empty"
        // skip path below.
        console.log(
          `[bridge:debug] incoming msg from=${msg.key.remoteJid} id=${msg.key.id} ` +
          `fromMe=${msg.key.fromMe} pushName=${msg.pushName} ` +
          `message_keys=${JSON.stringify(Object.keys(msg.message ?? {}))}`
        );
