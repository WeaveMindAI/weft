...
export class WebhookManager {
  ...
  async emit(event, data) {
    // Push to SSE subscribers. Per the SSE spec, the `event:` line
    // sets the event name (defaults to "message" when absent). The
    // weft-listener filters on this name; without an explicit `event:`
    // line every event would be dispatched as "message" and the
    // listener's per-subscription filter (`event_name = "message.received"`)
    // would reject every payload.
