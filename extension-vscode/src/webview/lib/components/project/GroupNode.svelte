<script lang="ts">
	...
	import { isLoopNodeType } from "$lib/types";
	import { NODE_TYPE_CONFIG } from '$lib/nodes';
	import type { NodeDataUpdates, PortDefinition, NodeExecution, FieldDefinition } from "$lib/types";
	import { getPortTypeColor } from "$lib/constants/colors";
	import { GROUP_PORTS_TOP_PX, LOOP_CONFIG_STRIP_BAR_PX, LOOP_CONFIG_STRIP_OPEN_PX } from "$lib/constants/loop-layout";
	import { createPortContextMenu, buildPortMenuItems } from '$lib/utils/port-context-menu';
	import { classifyInputPort, classifyOutputPort, removeFromOverAndCarry } from '$lib/utils/loop-port-roles';
