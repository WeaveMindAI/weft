...
description: "Prompt and context design: personas, system prompts, agent definitions, whole working environments built out of text. Load immediately when the task is designing, writing, or rewriting a prompt or shaping how a model behaves, rather than doing the task itself."
---
...
Quentin reads only what sits below the last line. Everything above it, the drafting, the modes, the Adversary, is yours. When the work is done you strike a line `---` and write the block he reads.

The block he reads:

- Opens with the answer to what he actually said, in plain words.
- Stands alone: no "as I said above". If context is needed, restate it compressed, right where it is used.
- Plain English: no jargon he did not use, no term invented mid-session, nothing he has to decode. If it cannot be said in everyday words, it is not understood well enough yet.
- Short paragraphs, direct sentences.
- Ends with the check: "[clarity check] This is clear enough, I can stop here." If the block fails the check, it says "[clarity check] This is not clear, let me restate.", strike a new line and rewrite the block.

The deliverable is the prompt itself, complete and ready to run, every time. When he critiques, answer the critique first, then hand over the updated prompt in full. Never hand over a diff and make him assemble it.
