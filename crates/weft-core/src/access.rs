...
//! - the RUNTIME grants the access (the input is empty or the
//!   `__PLATFORM__` sentinel): the runtime answers with a credential for its
//!   configured key, and may bound or refuse its use. The runtime may also
//!   answer with a RELAY address: calls on its credential are then sent
//!   there (the relay holds the actual provider relationship) instead of to
//!   the provider directly; the metered client does that routing, the node
//!   never sees it.

use serde::{Deserialize, Serialize};

/// The key-input sentinel meaning "the runtime grants access on its
/// configured key for this provider". The editor writes it when
...
/// runtime-granted access) where the runtime routes calls on it.
///
...
/// runtime-managed credential for one is not the node's concern; the
...
pub struct ProviderAccess {
    provider: String,
    credential: String,
    /// A runtime-granted access may carry a relay address: calls on its
    ...
    origin: AccessOrigin,
    /// How long the provider work on this access may run: a runtime-granted
...
impl ProviderAccess {
    ...
    /// The runtime's relay for calls on this access, when it routes
...
/// `None` = the runtime should grant access on its own. Pure; the context
/// method wraps it. Empty, whitespace-only, and the sentinel all mean
/// "runtime-granted" (a blank field is never a real key, so it must not be
...
/// runtime instead of being sent upstream as if it were a key.
...
mod tests {
    ...
    fn user_key_classification() {
        ...
        // the runtime instead of being sent upstream as a key.
        assert_eq!(user_key_of(Some(" sk-abc ")), Some("sk-abc"));
        assert_eq!(
    ...
    /// runtime-granted one, where its calls are routed. The node only
