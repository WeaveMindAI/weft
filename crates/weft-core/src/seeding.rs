...
use crate::project::{boundary_in_id, GroupBoundaryRole, GroupKind, NodeDefinition, ProjectDefinition};
...
pub fn starting_parameters(
    ...
    if saved.fire.is_none() {
        for (node, outcome) in outcomes.iter().filter(|(node, _)| authored.nodes.contains(*node)
            && planned.origins.contains_key(*node)) {
            if let Some(payload) = &outcome.fire {
                anyhow::ensure!(saved.fire.is_none(), "seed history contains more than one triggering use case");
                saved.fire = Some((node.clone(), payload.clone()));
            }
        }
    }
            ...
            && outcomes.get(root).is_some_and(|old| !old.used_backups.is_empty())))
    {
        ...
        for root in &roots {
            let node = project.nodes.iter().find(|node| &node.id == root).expect("selected root");
            if node.features.is_trigger { continue; }
            let enclosing_loop = project.groups.iter().find(|group| matches!(group.kind, GroupKind::Loop { .. })
                && (node.scope.contains(&group.id) || node.group_boundary.as_ref().is_some_and(|b| b.group_id == group.id)));
            let start = enclosing_loop.map(|group| group.id.clone())
                .or_else(|| node.group_boundary.as_ref().filter(|boundary| boundary.role == GroupBoundaryRole::In)
                    .map(|boundary| boundary.group_id.clone()))
                .unwrap_or_else(|| root.clone());
            saved.from.entry(start).or_default();
        }
    }
    for (id, ports) in saved.from.iter_mut().chain(saved.group.iter_mut().map(|(id, ports)| (&*id, ports))) {
        let entry = crate::project::selection::start_node(project, id).map_err(anyhow::Error::msg)?;
        let node = project.nodes.iter().find(|node| node.id == entry).expect("validated start");
        ...
        for edge in project.edges.iter().filter(|edge| edge.target == entry && authored.edges.contains(&edge.id)
            && !authored.nodes.contains(&edge.source) && planned.origins.contains_key(&edge.source))
        {
            let port = edge.target_handle.as_deref().unwrap_or("default");
            let source_port = edge.source_handle.as_deref().unwrap_or("default");
            let source_type = project.nodes.iter().find(|node| node.id == edge.source)
                .and_then(|node| node.outputs.iter().find(|port| port.name == source_port))
                .ok_or_else(|| anyhow::anyhow!("seed wire names undeclared output '{}.{source_port}'", edge.source))?;
            let mut values = Vec::new();
            for output in history.iter().filter(|output| output.node == edge.source && output.port == source_port
                && output.frames.is_empty() && !output.closed) {
...
pub fn seed_plan(
    ...
    for node in &project.nodes {
        let Some(old) = outcomes.get(&node.id) else { invalid.insert(node.id.clone()); continue };
        if hashes.get(&node.id) != Some(&old.slice_hash) {
            invalid.insert(node.id.clone());
            warnings.push(format!("cannot seed '{}': its code or dependencies changed; reuse stops before it", node.id));
        }
        if old.emitted_live_handle {
            invalid.insert(node.id.clone());
        }
        if selection.nodes.contains(&node.id) && !old.used_backups.is_empty()
            && !starting_inputs.contains_key(&node.id) && !roots.contains(&node.id)
        {
        ...
        if crate::project::selection::is_ordinary_boundary(project, &node.id)
            && selection.boundary_ports.get(&node.id).is_some_and(|ports| !ports.is_subset(&old.boundary_ports))
        { invalid.insert(node.id.clone()); }
        if let Some(authored) = starting_inputs.get(&node.id) {
    ...
    let starts: Vec<_> = spec.emit.keys().chain(spec.fire.iter().map(|(node, _)| node)).cloned().collect();
    let forced = RunSelection::downstream_nodes(project, &starts);
    invalid.extend(forced);
    invalid.extend(project.nodes.iter().filter(|n| !cap.nodes.contains(&n.id)).map(|n| n.id.clone()));
    // Dependency selections include control paths and whole loops, without
    // expanding unrelated ports of ordinary groups.
    loop {
        let previous = invalid.len();
        for node in &project.nodes {
            let dependencies = RunSelection::dependencies(project, std::slice::from_ref(&node.id));
            if dependencies.nodes.iter().any(|id| selection.nodes.contains(id) && invalid.contains(id)
                && !crate::project::selection::is_ordinary_boundary(project, id)) {
                invalid.insert(node.id.clone());
            }
        }
        for group in project.groups.iter().filter(|g| matches!(g.kind, GroupKind::Loop { .. })) {
            let members: Vec<_> = project.nodes.iter().filter(|n| n.scope.contains(&group.id)
                || n.group_boundary.as_ref().is_some_and(|b| b.group_id == group.id)).collect();
            let origins: BTreeSet<_> = members.iter().filter_map(|n| outcomes.get(&n.id).map(|o| o.origin)).collect();
            if origins.len() != 1 || members.iter().any(|n| invalid.contains(&n.id)) {
                invalid.extend(members.iter().map(|n| n.id.clone()));
            }
        }
        if invalid.len() == previous { break; }
    }
    let relevant = selection.history_nodes(project);
    let origins: BTreeMap<_, _> = outcomes.iter().filter(|(id, _)| relevant.contains(*id) && hashes.contains_key(*id) && !invalid.contains(*id))
        .map(|(id, result)| (id.clone(), result.origin)).collect();
    ...
    for (node, ports) in &starting_inputs {
        let (Some(origin), Some(old)) = (origins.get(node), outcomes.get(node)) else { continue };
        for port in ports.keys().filter(|port| !old.used_backups.contains_key(*port)) {
            warnings.push(format!(
                "'{node}.{port}' keeps the value run {origin} gave it; the value supplied at this start is a backup \
    ...
    for node in starting_inputs.keys().filter(|node| selection.nodes.contains(*node)) {
        if let Some(old) = outcomes.get(node).filter(|old| !old.used_backups.is_empty() && hashes.get(node) == Some(&old.slice_hash)) {
            let inputs = execution.input.entry(node.clone()).or_default();
            for (port, value) in &old.used_backups {
                if !inputs.contains_key(port) {
                    inputs.insert(port.clone(), value.clone());
                    execution.input_origins.entry(node.clone()).or_default().insert(port.clone(), old.backup_origins.get(port).copied().unwrap_or(old.origin));
                }
            }
        }
    }
...
/// The nodes of a folded run whose outcome a child may inherit: every
/// record the node has is Completed or Skipped (a node fired in three
/// loop iterations must have settled in all three), and none is parked
/// on a suspension. A node with no record never ran and is not here.
pub fn inheritable_nodes(project: &ProjectDefinition, snapshot: &ExecutionSnapshot) -> HashSet<String> {
    let mut eligible: HashSet<_> = snapshot
        .executions
        .iter()
        .filter(|(_, records)| {
            !records.is_empty()
                && records.iter().all(|r| {
                    matches!(r.status, NodeExecutionStatus::Completed | NodeExecutionStatus::Skipped)
                })
        })
        .map(|(id, _)| id.clone())
        .collect();
    for node in project.nodes.iter().filter(|node| crate::project::selection::is_ordinary_boundary(project, &node.id)) {
        if snapshot.selection.as_ref().is_none_or(|selection| selection.nodes.contains(&node.id))
            && skipped_scope(project, snapshot, node, &Vec::new())
        { eligible.insert(node.id.clone()); }
    }
    let loops: HashSet<_> = project.groups.iter().filter(|group| matches!(group.kind, GroupKind::Loop { .. }))
        .map(|group| group.id.as_str()).collect();
    for group in &loops {
        let Some(entry) = project.nodes.iter().find(|node| node.id == boundary_in_id(group)) else { continue };
        if entry.scope.iter().any(|scope| loops.contains(scope.as_str())) { continue; }
        let members: Vec<_> = project.nodes.iter().filter(|node| node.scope.iter().any(|scope| scope == group)
            || node.group_boundary.as_ref().is_some_and(|boundary| &boundary.group_id == group)).collect();
        let complete = complete_loop(project, snapshot, group, &Vec::new(), &loops)
            && members.iter().all(|node| snapshot.executions.get(&node.id).is_none_or(|records|
                records.iter().all(|record| matches!(record.status, NodeExecutionStatus::Completed | NodeExecutionStatus::Skipped))));
        for node in members {
            if complete { eligible.insert(node.id.clone()); } else { eligible.remove(&node.id); }
        }
    }
    eligible
}

fn skipped_scope(project: &ProjectDefinition, snapshot: &ExecutionSnapshot, node: &NodeDefinition, frames: &crate::frames::LoopFrames) -> bool {
    node.scope.iter().chain(node.group_boundary.iter().filter(|boundary| boundary.role == GroupBoundaryRole::Out).map(|boundary| &boundary.group_id))
        .any(|group| {
            let gate = boundary_in_id(group);
            let Some(definition) = project.nodes.iter().find(|node| node.id == gate) else { return false };
            let depth = definition.scope.iter().filter(|scope| project.groups.iter()
                .any(|group| &group.id == *scope && matches!(group.kind, GroupKind::Loop { .. }))).count();
            snapshot.executions.get(&gate).is_some_and(|records| records.iter().any(|record|
                record.frames.as_slice() == &frames[..depth.min(frames.len())] && record.status == NodeExecutionStatus::Skipped))
        })
}

fn complete_loop(project: &ProjectDefinition, snapshot: &ExecutionSnapshot, group: &str,
    frames: &crate::frames::LoopFrames, loops: &HashSet<&str>) -> bool
{
    let Some(entry) = project.nodes.iter().find(|node| node.id == boundary_in_id(group)) else { return false };
    if skipped_scope(project, snapshot, entry, frames) { return true; }
    ...
    for index in &instance.launched {
        let mut iteration = frames.clone();
        iteration.push(crate::frames::LoopIteration { index: *index });
        for node in &project.nodes {
            let own_boundary = node.group_boundary.as_ref().filter(|boundary| boundary.group_id == group);
            if own_boundary.is_some_and(|boundary| boundary.role == GroupBoundaryRole::In) { continue; }
            let directly_inside = node.scope.iter().rev().find(|scope| loops.contains(scope.as_str())).is_some_and(|scope| scope == group);
            if !directly_inside && own_boundary.is_none() { continue; }
            if skipped_scope(project, snapshot, node, &iteration) { continue; }
            if let Some(boundary) = node.group_boundary.as_ref().filter(|boundary| boundary.group_id != group && loops.contains(boundary.group_id.as_str())) {
                if boundary.role == GroupBoundaryRole::Out { continue; }
                if !complete_loop(project, snapshot, &boundary.group_id, &iteration, loops) { return false; }
                continue;
            }
            if !snapshot.executions.get(&node.id).is_some_and(|records| records.iter().any(|record|
                record.frames == iteration && matches!(record.status, NodeExecutionStatus::Completed | NodeExecutionStatus::Skipped)))
            { return false; }
        }
    }
    true
...
mod tests {
    ...
    fn saved_seeded_starts_preserve_the_cut_and_external_values_only() {
        ...
        for (spec, source, entry) in [
            ...
            let planned = SeedPlan { origins: [(source.into(), Color::nil())].into(), selection: authored.clone(), warnings: vec![] };
            ...
            assert_eq!(saved.starting_inputs(&project)[entry]["in"], json!("original input"));
        ...
        let planned = SeedPlan { origins: [("a".into(), Color::nil())].into(), selection: authored.clone(), warnings: vec![] };
    ...
    fn saved_whole_run_preserves_original_roots_and_recomputes_their_descendants() {
        for trigger in [false, true] {
            ...
            if trigger {
                previous.get_mut("a").unwrap().fire = Some(json!({"event": "original wake"}));
            } else {
                previous.get_mut("a").unwrap().used_backups.insert("in".into(), json!("original input"));
            }
            let planned = SeedPlan { origins: [("a".into(), Color::nil())].into(), selection: authored.clone(), warnings: Vec::new() };
            ...
            assert!(fresh.selection.nodes.contains("a") && fresh.selection.nodes.contains("b") && fresh.selection.nodes.contains("d"));
        }
    }
    ...
    fn saved_whole_group_start_keeps_unwired_members() {
        ...
        let authored = RunSelection::whole(&project);
        let mut previous = outcomes(&project);
        previous.get_mut("l__in").unwrap().used_backups.insert("in".into(), json!("saved group input"));
        let planned = SeedPlan {
            origins: [("l__in".into(), Color::nil())].into(),
    ...
    fn a_new_trigger_run_does_not_inherit_an_unrelated_trigger_use_case() {
        ...
        let mut previous = outcomes(&project);
        previous.get_mut("a").unwrap().fire = Some(json!({"old": "event"}));
    ...
    fn outcomes(project: &ProjectDefinition) -> BTreeMap<String, SeedOutcome> {
        identity(project).slice_hashes(project).unwrap().into_iter().map(|(id, slice_hash)| {
            (id, SeedOutcome { origin: Color::nil(), slice_hash, used_backups: BTreeMap::new(), backup_origins: BTreeMap::new(), absent_ports: BTreeSet::new(), boundary_ports: BTreeSet::new(), emitted_live_handle: false, fire: None })
        }).collect()
    }
    ...
    fn nested_live_handles_keep_the_producer_and_its_consumers_out_of_reuse() {
        ...
        previous.get_mut("a").unwrap().emitted_live_handle = crate::weft_type::WeftType::contains_bus_handle(&handle);
        ...
        assert!(!planned.origins.contains_key("a"));
        assert!(!planned.origins.contains_key("b"));
        assert!(!planned.origins.contains_key("body"));
        assert!(planned.origins.contains_key("x"));
    }
    ...
    fn reuse_keeps_history_before_an_explicit_start_without_widening_the_run() {
        ...
        assert!(result.origins.contains_key("a"));
        assert!(result.origins.contains_key("b"));
        assert!(result.origins.contains_key("c"));
        assert!(!result.origins.contains_key("x"), "unrelated history cannot become part of this use case");
        assert!(result.selection.nodes.is_empty(), "unchanged selected work is reused too");
        assert!(!result.selection.nodes.contains("a"));
    }
    ...
    fn a_seed_boundary_before_a_loop_runs_the_whole_loop_and_its_consumers() {
        ...
        assert_eq!(result.selection.nodes, BTreeSet::from(["body".into(), "l__in".into(), "l__out".into(), "d".into()]));
    ...
    fn an_explicit_start_reuses_unchanged_inputs_and_reruns_a_changed_used_backup() {
        ...
        assert!(result.origins.contains_key("b"), "an unused backup does not change the effective input");
        assert!(result.origins.contains_key("a"), "upstream history remains reusable");
        ...
        old.get_mut("b").unwrap().used_backups.insert("in".into(), json!("old"));
        let result = seed_plan(&p, &identity(&p), &selected, &spec, &old, &[], &[]).unwrap();
        assert!(!result.origins.contains_key("b"));
        assert!(!result.origins.contains_key("d"));
        assert_eq!(result.selection.input["b"]["in"], json!("new"));
        old.get_mut("b").unwrap().used_backups.insert("in".into(), json!("new"));
        let result = seed_plan(&p, &identity(&p), &selected, &spec, &old, &[], &[]).unwrap();
        assert!(result.origins.contains_key("b"), "the same used backup is reusable");
    ...
    fn planned(parent: &ProjectDefinition, child: &ProjectDefinition, until: &[&str], missing: &[&str]) -> SeedPlan {
        ...
        for node in missing { old.remove(*node); }
    ...
    fn config_change_recomputes_only_its_dependencies_and_whole_loop() {
        ...
        assert_eq!(planned(&parent, &child, &[], &[]).selection.nodes,
            ["b", "c", "l__in", "body", "l__out", "d"].map(String::from).into_iter().collect());
    }
    ...
    fn new_wire_invalidates_its_consumer_and_dependents() {
        ...
        assert_eq!(planned(&parent, &child, &[], &[]).selection.nodes,
            ["c", "l__in", "body", "l__out", "d"].map(String::from).into_iter().collect());
    }
    ...
    fn new_or_unfinished_nodes_cannot_reuse_older_history() {
        ...
        assert_eq!(planned(&parent, &child, &[], &[]).selection.nodes, BTreeSet::from(["x".into()]));
    ...
    fn exclusive_and_inclusive_seed_caps_do_not_widen_the_execution_cut() {
        ...
        assert!(until.origins.contains_key("b"));
        assert_eq!(until.selection.nodes, BTreeSet::from(["c".into()]));
        let before = seed_plan(&p, &identity(&p), &selection, &RunSpec::whole("test"), &old, &[], &["b".into()]).unwrap();
        assert!(!before.origins.contains_key("b"));
        assert_eq!(before.selection.nodes, BTreeSet::from(["b".into(), "c".into()]));
    }
    ...
    fn used_ancestor_backup_is_copied_only_to_an_explicit_start() {
        ...
        let mut old = outcomes(&p);
        old.get_mut("b").unwrap().used_backups.insert("in".into(), json!("old"));
        old.get_mut("c").unwrap().used_backups.insert("in".into(), json!("interior"));
        ...
        assert_eq!(result.selection.input["b"]["in"], json!("old"));
        assert_eq!(result.selection.input_origins["b"]["in"], old["b"].origin);
        assert!(!result.selection.input.contains_key("c"), "seed does not invent an interior starting point");
        assert!(!result.selection.input.contains_key("a") || result.selection.input["a"].is_empty());
    }
    ...
    fn expanding_before_an_old_start_recomputes_its_backup_dependent_result() {
        ...
        old.get_mut("b").unwrap().used_backups.insert("in".into(), json!("old manual input"));
        ...
        assert!(planned.origins.contains_key("x"));
        assert!(!planned.origins.contains_key("b"));
        assert!(!planned.origins.contains_key("d"));
        assert!(!planned.selection.input.contains_key("b"));
    ...
    fn an_unwired_loop_member_is_reused_or_rerun_with_its_whole_loop() {
        ...
        for node in ["lonely", "body", "l__in", "l__out", "d"] { assert!(rerun.selection.nodes.contains(node)); }
    }
    ...
    fn a_loop_cannot_mix_results_from_different_origins() {
        let p = program("b", WIRES);
        let mut old = outcomes(&p);
        old.get_mut("body").unwrap().origin = Color::new_v4();
        let result = seed_plan(&p, &identity(&p), &RunSelection::whole(&p), &RunSpec::whole("test"), &old, &[], &[]).unwrap();
        for node in ["body", "l__in", "l__out", "d"] { assert!(result.selection.nodes.contains(node)); }
    }
