...
/// (generated by `codegen::emit_warmup_crate` before the base build). The base
/// Dockerfile COPYs it to `/work` and compiles it into the shared `/weft/target`,
/// precooking the rlibs every worker reuses. Kept out of the repo's tracked tree
/// (a dot-dir) since it is a generated build artifact.
...
/// builder base has already compiled the whole weft workspace
/// `--release` into `/weft/target`, and every worker crate's fixed
/// dependency set (weft-engine + weft-core + weft-broker-client +
/// weft-platform-traits as path deps, plus tokio/serde/clap/... ) is a
/// subset of that workspace, pinned by the SAME `Cargo.lock`. So this
/// per-project build:
///
/// - points `CARGO_TARGET_DIR` at the baked `/weft/target`, so cargo
///   finds every engine + dependency rlib already present and
///   fingerprinted-fresh and compiles ONLY the project-specific crates
///   (the `pkg_<node>` packages + the top worker crate). The baked dir
///   is this build's own copy-on-write image layer, so writing the
///   project artifacts into it is isolated to this build (no
///   cross-build race), and
...
///
/// There is NO `--mount=type=cache` over the target dir: a cache mount
/// would SHADOW the baked `/weft/target`, hiding the precompiled engine
/// and forcing the cold compile this design exists to avoid. The
/// registry cache mount is kept (crates.io artifacts are immutable per
/// version+features, so it is shared and safe) only to speed fetching
/// any NEW node-specific deps a project pulls beyond the workspace set;
/// the engine + workspace deps are already compiled, not re-fetched.
///
...
const CARGO_BUILD_RUN_FRAGMENT: &str = concat!(
    "ENV CARGO_TARGET_DIR=/weft/target\n",
    ...
    "    && cargo build --release \\\n",
    "    && cp /weft/target/release/{{binary_name}} /worker\n",
);
...
mod tests {
    ...
    fn non_debian_runtime_falls_back_to_from_scratch_template() {
        ...
        // Both templates point cargo at the baked workspace target dir
        // and seed the worker lock from the workspace lock so the
        // engine + dep rlibs are reused instead of recompiled.
        assert!(
            out.body.contains("CARGO_TARGET_DIR=/weft/target"),
            "fallback template targets the baked workspace dir: {}",
            out.body
        );
        assert!(
    ...
    fn default_debian_runtime_uses_prebuilt_base() {
        ...
        // The prebuilt path reuses the base's precompiled engine: it
        // targets the baked workspace dir and seeds the worker lock
        // from the workspace lock, so only project crates compile.
        assert!(
            out.body.contains("CARGO_TARGET_DIR=/weft/target"),
            "prebuilt path targets the baked workspace dir: {}",
            out.body
        );
        assert!(
            out.body.contains("cp /weft/Cargo.lock /work/Cargo.lock"),
        ...
        // The binary is copied from the baked target dir, not a local
        // /work/target.
        assert!(
            out.body.contains("cp /weft/target/release/worker_test /worker"),
            "prebuilt path copies the binary from the baked target dir: {}",
            out.body
        );
    }
}
