...
async fn an_include_runs_as_its_group_and_a_loop_goes_stale_whole() -> anyhow::Result<()> {
    ...
    reused.completed()?;
    anyhow::ensure!(inherited(&reused.replay, "triage.up") && inherited(&reused.replay, "triage.unrelated"),
    ...
    for (endpoint, member, source, after) in [
        ...
        let run = SettledRun::observe(project.dispatcher(), color_of(&stdout)?).await?;
    ...
    SettledRun::observe(project.dispatcher(), color_of(&stdout)?).await?.completed()?.assert_untouched("src")?.assert_untouched("shout")?;

    // A `--from` inside the include takes values at that node.
    let stdout = project
        .weft(&["run", "--json", "--from", r#"triage.up={"text":"inner"}"#, "--target", "shout"])
        .await?;
    SettledRun::observe(project.dispatcher(), color_of(&stdout)?).await?.completed()?.assert_input("shout", "data", &json!("INNER"))?
    ...
    SettledRun::observe(project.dispatcher(), color_of(&stdout)?).await?.completed()?.assert_input("shout", "data", &json!("ENTRY"))?.assert_untouched("src")?;
    ...
    SettledRun::observe(project.dispatcher(), color_of(&stdout)?).await?.completed()?.assert_completed("triage.first")?
    ...
    SettledRun::observe(project.dispatcher(), color_of(&stdout)?).await?.completed()?.assert_completed("triage.up")?
    ...
    SettledRun::observe(project.dispatcher(), color_of(&stdout)?).await?.completed()?.assert_skipped("triage.up")?.assert_skipped("shout")?;

    let stdout = project.weft(&["run", "--json", "--group", r#"echoes={"values":[3,5]}"#]).await?;
    SettledRun::observe(project.dispatcher(), color_of(&stdout)?).await?.completed()?.assert_loop_iterations("echoes", 2)?
        .assert_untouched("nums")?.assert_untouched("total")?;
    let stdout = project.weft(&["run", "--json", "--from", r#"echoes={"values":[3,5]}"#]).await?;
    SettledRun::observe(project.dispatcher(), color_of(&stdout)?).await?.completed()?.assert_input("total", "data", &json!([6,10]))?
    ...
    let settled = SettledRun::observe(project.dispatcher(), color_of(&stdout)?).await?;
    settled.completed()?.assert_loop_iterations("echoes", 2)?.assert_input("total", "data", &json!([2, 4]))?;
    anyhow::ensure!(inherited(&settled.replay, "nums"), "the loop's source is inherited");
    anyhow::ensure!(!inherited(&settled.replay, "echoes.step"), "the loop body ran again");
...
async fn frozen_carved_inputs_survive_seed_cleanup_and_explicit_cut_repair() -> anyhow::Result<()> {
    ...
    SettledRun::observe(project.dispatcher(), seed).await?.completed()?;
    let out = project.weft(&["run", "--json", "--seed", "--from", "triage.up", "--target", "shout"]).await?;
    let carved = color_of(&out)?;
    SettledRun::observe(project.dispatcher(), carved).await?.completed()?.assert_input("shout", "data", &json!("HELLO"))?;
    ...
    SettledRun::observe(project.dispatcher(), color_of(&out)?).await?.completed()?
        .assert_input("shout", "data", &json!("HELLO"))?.assert_untouched("src")?;
    let path = project.dir().join("triage.weft");
    project.write_file("triage.weft", &std::fs::read_to_string(path)?.replace("up =", "upper =").replace("up.text", "upper.text").replace("up.out", "upper.out"))?;
...
async fn a_frozen_example_runs_with_a_new_human_answer_and_explicit_acceptance() -> anyhow::Result<()> {
    ...
    SettledRun::observe(&disp, first).await?.completed()?.assert_input("gate", "data", &json!(true))?;
    ...
    SettledRun::observe(&disp, parked).await?.completed()?;
...
async fn wake_resolves_a_timer_and_refuses_a_form() -> anyhow::Result<()> {
    ...
    SettledRun::observe(&disp, held).await?.completed()?.assert_input("late", "data", &json!("hello"))?;
