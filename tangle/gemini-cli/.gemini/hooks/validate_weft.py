#!/usr/bin/env python3
"""AfterTool hook: the compiler answers every edit, at the right tier.
...
keeps tier-1 findings: severity "error" minus the rule-runtime slug. Gemini runs `AfterTool` hooks
synchronously and feeds what they return back into the model's context,
so the finding lands in the same turn as the edit.
...
Silence means clean: an empty JSON object when there is nothing to say,
...
def main() -> int:
    """Gemini hands the hook JSON on stdin and reads pure JSON on stdout.

    Nothing but JSON may reach stdout, so every early return prints an empty
    object. The edited path arrives in the tool's own arguments."""
    try:
