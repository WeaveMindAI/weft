...
1. Read the manual, `.windsurf/skills/weft-node-authoring/SKILL.md` in this project: the current anatomy, metadata schema, and Rust pattern; it beats what you remember.
...
4. Write the files:
   - `metadata.json`: [the contract] verbatim, plus presentation. A trigger's metadata also gets `firesWith`: EVERY field the wake payload can carry, with `?` on the ones that only sometimes arrive, so a real firing (and a hand-typed `weft run --fire`) is checked against that shape before `run` starts. The check is exact: a firing carrying a field you never named is refused just like one missing a field you required. Name only what you fan onto ports and the trigger dies the first time the provider sends anything else; when the payload comes from a connection's events, copy the names from `events.<topic>.fields` in the service's recipe. Leave it off only when the node genuinely has nothing to name (it reads its own connection instead of the payload, or its fields are per-instance config the author typed in), and note why in the report.
