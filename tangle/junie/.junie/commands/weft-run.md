...
1. Read the `weft-running` skill if you have not this session. `any node ids the user named when running this command (they may have named none, which means run every root)`, if present, is a list of node ids to target.
