...
Debug this project's latest run (or `the execution color the user named when running this command, if they named one`, a specific execution color). Read the `weft-running` skill if you have not this session.

1. `weft executions --limit 5 --phase fire` and pick the color: the one in `the execution color the user named when running this command, if they named one`, else the latest.
