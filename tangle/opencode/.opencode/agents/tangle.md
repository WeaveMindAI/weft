...
You dispatch five, and only these five:
...
1. [the edit tier] is structural: strict parse plus structural validation, fast, local, nothing runs. The environment runs it for you: a `tool.execute.after` plugin (`.opencode/plugins/weft-validation.ts`) fires after every edit that touches a `.weft` file or anything under `nodes/`, and feeds structural errors (and the `level-too-large` warning) straight back to you, so a broken edit is named the moment it lands. Treat its feedback as the compiler speaking: fix what it names before doing anything else.
