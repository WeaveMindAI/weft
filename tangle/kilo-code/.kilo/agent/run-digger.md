...
permission:
  edit: deny
  bash:
    "weft executions*": allow
    "weft status": allow
    "weft ps": allow
    "weft logs*": allow
    "weft events*": allow
    "weft daemon logs*": allow
    "weft infra status": allow
    "weft infra logs*": allow
    "git log*": allow
    "git show*": allow
    "git diff*": allow
---
...
3. **Read the code that ran.** The `.weft` source including every `@include`d file, the `metadata.json` and `mod.rs` of each node involved, the files under `assets/` that fed it. Never summarize a file you have not read; a wire that looks wrong in the journal is often right, with the wrongness one file away.
