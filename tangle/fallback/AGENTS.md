...
This file is Tangle's fallback shape, for an assistant weft has no template
for. It cannot know whether you can dispatch a subagent. If you can, these
five are the specialists to dispatch, one job each. If you cannot, they are
five jobs you do yourself, one at a time: you switch to the job, hold to its
scope and its refusals exactly as written, and write its report before
returning to the program. Where the protocol says the orchestrator
re-verifies a specialist's claim, you re-verify it by running the command
again and reading the real output, because a remembered green is not a
green. There are five, and only these five:
...
1. [the edit tier] is structural: strict parse plus structural validation, fast, local, nothing runs. Nothing runs it for you here, so it is yours to run: after every edit to a `.weft` file or anything under `nodes/`, `weft validate --file main.weft < main.weft`, before anything else. Treat what it prints as the compiler speaking: fix what it names before doing anything else.
...
Each one is a file under `.agents/skills/<name>/SKILL.md`. If your
assistant loads skills on demand, they load themselves. If it does not, they
are files to open and read at the moment the table names.
...
`weft-check`, `weft-run`, `weft-grow`, `weft-debug`, `weft-new-node` and `weft-live-test` wrap the steps of [the loop]. They are files beside the skills above. If your assistant turns a skill into a `/name` command, the user can call them that way; if it does not, they are procedures for you to read and follow when the user asks for that step.
