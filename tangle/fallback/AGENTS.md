...
`weft-check`, `weft-run`, `weft-debug`, `weft-new-node` and `weft-live-test` wrap the steps of [the loop]. They are files beside the skills above. If your assistant turns a skill into a `/name` command, the user can call them that way; if it does not, they are procedures for you to read and follow when the user asks for that step.
