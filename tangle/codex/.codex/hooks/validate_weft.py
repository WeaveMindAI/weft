...
`hookSpecificOutput.additionalContext`, which Codex injects into the
model's context at the next safe point.
...
def main() -> int:
    """Codex hands the hook JSON on stdin and reads JSON back on stdout.

    `PostToolUse` carries the tool's own arguments in `tool_input`; the
    edited path lives there under whichever key the tool used. Feedback
    reaches the model as `additionalContext`. The edit already landed, so
    this never blocks: it reports."""
    try:
