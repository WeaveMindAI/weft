...
Projects created with `weft new <name> --assistant cursor` (shorthand `cu`; the choice is remembered, so plain `weft new` installs it too once picked) have their Tangle persona (`.cursor/`) COPIED into the project, so a project keeps the version that created it. After the `./setup.sh`, `weft tangle update` inside a project re-copies its Tangle from [the checkout]; run it in each project the user wants on the new version. It replaces every file Tangle owns and leaves anything the assistant wrote beside them alone, so the diff is worth reading.
