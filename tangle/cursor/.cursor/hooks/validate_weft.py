...
After every file edit this runs `weft validate` (strict pipeline) and
keeps tier-1 findings: severity "error" minus the rule-runtime slug. The finding is returned as
`additional_context`, which Cursor injects into the conversation after the
tool result. It is `postToolUse` rather than `afterFileEdit` deliberately:
`afterFileEdit` fires on the same edits but accepts no output fields, so a
finding reported there would reach nobody.
...
def main() -> int:
    """Cursor hands the hook JSON on stdin and reads JSON back on stdout.

    `postToolUse` carries the tool's own arguments in `tool_input`; the
    agent's file-writing tool is `Write`. Feedback reaches the model through
    `additional_context`; the edit already happened, so there is nothing to
    block, and an empty object is the silent case."""
    try:
    ...
    if blocking:
        print(json.dumps({"additional_context": blocking}))
    else:
        print("{}")
    return 0
