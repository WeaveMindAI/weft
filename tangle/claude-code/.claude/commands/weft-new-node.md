...
argument-hint: <what the node should do, in plain words>
---

This is the expert path for a node the user wants built by name. `$ARGUMENTS` is the capability, in plain words. (In normal work you never need this command: missing nodes are dispatched by [the loop] itself.)
...
3. Ask the user one question, only if it changes the contract: hand or dispatch?
   - **Dispatch**: design the typed contract (one job, exact ports with types and exposure), then dispatch a `node-smith` with [the brief], run [the review] on the report against the checklist, redispatch on failure, and report the landed node.
