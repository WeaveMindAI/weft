...
3. Ask the user one question, only if it changes the contract: hand or dispatch?
   - **Dispatch**: design the typed contract (one job, exact ports with types and exposure), then dispatch a `node-smith` with the brief, review its report against the checklist, redispatch on failure, and report the landed node.
