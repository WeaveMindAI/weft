...
1. [the edit tier] is structural: strict parse plus structural validation, fast, local, nothing runs. The environment runs it for you: a PostToolUse hook (`.claude/hooks/validate_weft.py`, registered in `.claude/settings.json`) fires after every Edit, Write or MultiEdit that touches a `.weft` file or anything under `nodes/`, and feeds structural errors straight back to you, so a broken edit is named the moment it lands. Treat its feedback as the compiler speaking: fix what it names before doing anything else.
