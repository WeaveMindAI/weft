...
The deliverable of a run is the firing of an [output node]: a node with `_is_output: true`, or a type that defaults to output. A run walks backwards from the [output node]s, so a branch wired to no output never executes.
