...
`/weft-check`, `/weft-run`, `/weft-grow`, `/weft-debug`, `/weft-new-node` and `/weft-live-test` wrap the steps of [the loop] for the user. They are skills like the thirteen above, and the difference is what you do with them: the thirteen are reference you read when the work calls for it, these six are procedures you carry out when the user asks for that step by name. Each one says which it is on its first line.
